...
export default function RiwayatTransaksiContent({ transactions }) {
  ...
  const totalFiltered = sortedTransactions.reduce((sum, t) => sum + t.total, 0);
    ...
    <div className="space-y-4 sm:space-y-6">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-2 lg:grid-cols-3 gap-2 sm:gap-4">
        <div className="bg-gradient-to-br from-red-50 to-rose-50 rounded-lg sm:rounded-xl p-3 sm:p-4 border border-red-200">
          <p className="text-red-600 text-xs sm:text-sm font-semibold mb-1">Total Transaksi</p>
          <p className="text-xl sm:text-2xl font-bold text-red-900">{sortedTransactions.length}</p>
        </div>
        <div className="bg-gradient-to-br from-emerald-50 to-teal-50 rounded-lg sm:rounded-xl p-3 sm:p-4 border border-emerald-200">
          <p className="text-emerald-600 text-xs sm:text-sm font-semibold mb-1">Total Revenue</p>
          <p className="text-lg sm:text-xl font-bold text-emerald-900 break-words">Rp {totalFiltered.toLocaleString("id-ID")}</p>
        </div>
        {sortedTransactions.length > 0 && (
          <div className="bg-gradient-to-br from-amber-50 to-orange-50 rounded-lg sm:rounded-xl p-3 sm:p-4 border border-amber-200 col-span-2 sm:col-span-1">
            <p className="text-amber-600 text-xs sm:text-sm font-semibold mb-1">Rata-rata</p>
            <p className="text-lg sm:text-xl font-bold text-amber-900 break-words">Rp {Math.round(totalFiltered / sortedTransactions.length).toLocaleString("id-ID")}</p>
          </div>
        )}
      </div>
      ...
      <div className="bg-white rounded-lg sm:rounded-xl p-4 sm:p-6 border border-gray-200 shadow-sm space-y-4">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 sm:gap-4">
          {/* Date Range */}
          <div>
            <label className="block text-xs sm:text-sm font-medium text-gray-700 mb-1 sm:mb-2">Dari Tanggal</label>
            <input
              type="date"
              value={startDate}
          ...
          <div>
            <label className="block text-xs sm:text-sm font-medium text-gray-700 mb-1 sm:mb-2">Sampai Tanggal</label>
            <input
              type="date"
              value={endDate}
        ...
        {/* Search */}
        <div>
          <label className="block text-xs sm:text-sm font-medium text-gray-700 mb-1 sm:mb-2">Cari Item / Nominal</label>
          <div className="relative">
            <Search size={16} className="absolute left-3 sm:left-4 top-2.5 sm:top-3 text-gray-400" />
            <input
              type="text"
      ...
      <div className="space-y-2 sm:space-y-4">
        {sortedTransactions.length > 0 ? (
          sortedTransactions.map(transaction => (
            <div key={transaction.id} className="bg-white rounded-lg sm:rounded-xl shadow border border-gray-200 overflow-hidden hover:shadow-md transition">
              <div 
              ...
              >
                <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2 sm:gap-4">
                  <div className="flex-1 min-w-0">
                    <p className="font-bold text-gray-900 text-sm sm:text-base">Transaksi #{transaction.id}</p>
                    <p className="text-xs sm:text-sm text-gray-600 mt-0.5 sm:mt-1">{transaction.date} • {transaction.time}</p>
                  </div>
                  <div className="flex items-center justify-between sm:justify-end gap-3 sm:gap-4">
                    <div className="text-right">
                      ...
                      <p className="text-xs sm:text-sm text-gray-600">{transaction.items.length} item</p>
                    </div>
                ...
                <div className="bg-gradient-to-b from-red-50 to-white p-3 sm:p-6 border-t">
                  ...
                  <div className="space-y-2 sm:space-y-3">
                    {transaction.items.map((item, idx) => (
                      <div key={idx} className="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-2 p-2 sm:p-3 bg-white rounded-lg border border-gray-100 text-xs sm:text-sm">
                        <div className="flex-1">
                          <p className="font-medium text-gray-900">{item.name}</p>
                          <p className="text-gray-600 text-xs sm:text-sm">{item.quantity} x {item.price}</p>
                        </div>
                        <p className="font-bold text-red-600">
                          Rp {(parseInt(item.price.replace(/\D/g, '')) * item.quantity).toLocaleString("id-ID")}
                        </p>
                      </div>
                    ))}
                  </div>
          ...
          <div className="bg-white p-8 sm:p-12 rounded-lg sm:rounded-xl shadow text-center border border-gray-200">
            <Calendar size={32} className="mx-auto text-gray-300 mb-3 sm:mb-4" />
            <p className="text-gray-500 text-sm sm:text-lg">Belum ada transaksi untuk periode ini</p>
          </div>
        )}
      </div>
    </div>
