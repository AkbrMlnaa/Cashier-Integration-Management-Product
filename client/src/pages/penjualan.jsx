...
export default function SalesContent({ transactions = []}) {
  ...
  data.forEach(t => {
    t.details.forEach(d => {
      if (!topMenus[d.product?.name]) topMenus[d.product?.name] = { qty: 0, revenue: 0 };
...
const flattenedData = data.flatMap(t =>
  t.details.map(d => ({
    id: d.transaction_id,
    menu: d.product?.name || '-',
    qty: d.quantity,
  ...
  return (
    <div className="space-y-4 sm:space-y-6">
      ...
      <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
        <div>
          <h1 className="text-2xl sm:text-3xl font-bold bg-gradient-to-r from-red-600 to-rose-600 bg-clip-text text-transparent">Analisis Penjualan</h1>
        ...
        <div className="bg-gradient-to-br from-red-50 to-rose-50 rounded-lg sm:rounded-xl p-4 sm:p-6 border border-red-200 shadow-sm hover:shadow-md transition">
          <div className="flex items-start justify-between gap-3">
            <div className="flex-1 min-w-0">
              <p className="text-red-600 text-xs sm:text-sm font-semibold mb-1">Total Pendapatan</p>
